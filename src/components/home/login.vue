<template>
  <div class="bg_white">
    <div id="login-content">
      ...
      <div v-show="loginType != 1" class="warn-2"> 请以来穗通 "注册账号"和"手机号" 登录</div>        
  ...
  methods: {
    ...
    loginIn() {
      ...
      // 不刷脸
        ...
        let _oid 
        if(this.loginType == 2){
          _oid = this.$refs.idCards.value
        }else if(this.loginType == 3){
          _oid = this.$refs.phones.value
        }
        api
          .checkUser({
            openId: this.$Utils.getLocalStorage("openId"),
...
<style scoped>
.warn-2{font-size:.26rem;text-align: center;color:#888;}
.wrapper {
  width: 83.3%;
  margin: 0 auto;
}
