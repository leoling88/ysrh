        ...
        <li v-for="(item, index) in newsData.lists">
          <div class="text">
            <router-link :to="{path:'../../article2/'+ openId +'/'+ listType +'/' +item.id}">
              ...
              <span class="img"><img :src="item.activityCover" alt=""></span>
              <span class="title">{{item.activityName}}</span>
              <span class="con">时间 : {{item.activityStartTime | formatDate}} 至 {{item.activityEndTime | formatDate}} </span></span>
              <span class="con">地点 : {{item.activityAddress}}</span>
            </router-link>
            <div v-if="item.effective == '0'">
              <span class="but"  v-if="item.sign == '1'">已报名</span>
              <span class="but col-blue" v-else @click="clickApply(item.id, item.activityName, item.activityAddress, item.activityStartTime)">我要报名</span>
            </div>
...
export default {
  ...
  submitApply(){    //点击报名
    if (!this.formatDate.phone) {
      ...
      api.RHApply({
        openId:this.openId,
        activityId:this.formatDate.classroomId,
      ...
      getUserActivityTitle(this.openId,this.formatDate.classroomId,3);//融合活动报名打标签
    }
