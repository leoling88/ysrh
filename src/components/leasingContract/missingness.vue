    ...
    <div class="lesing_one">
      ...
      <group class="group_style" v-if="formData.applicantType==1">
        <x-input
          ...
          ref="name"
          @on-blur="((val) => {blurinput(val,'renter')})"
        ></x-input>
        <x-input
          title="证件号码"
          text-align="right"
          ...
          ref="idCard"
           @on-blur="((val) => {blurinput(val,'rentNo')})"
        ></x-input>
        <!-- @on-change="changData" -->
        <!-- :required="true" -->
...
<!--         <popup-picker
    ...
    line-height: .8rem;"
        ...
        <x-input
          ...
          ref="houseArea"
          type="number"
          @on-blur="((val) => {blurinput(val,'houseArea')})"
  ...
  mounted() {
    ...
    if (this.$route.query.Type) {
      ...
      this.$route.query.Type == 1
        ? (this.placeholderText = "请填写房产证权号(选填)")
        : (this.placeholderText = "请填写房产证权号");
    }
        ...
        if (allCheckData) {
          if (this.formData.applicantType == 1) {
            if (!this.$refs.name.valid) {
              return "请输入正确的名字";
            } else if (this.formData.sex.length == 0) {
              return "请选择性别";
...
.two_content{margin-left:.3rem;border-top:.01rem solid #ccc;padding:.2rem .3rem .2rem 0;}
</style>
